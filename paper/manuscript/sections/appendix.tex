...
\input{tables/faiss_scale_stress}
\input{tables/cross_engine_scale_stress}
...
\begin{itemize}[leftmargin=*]
  ...
  \item Claim: the S3 multi-evidence quality-first pgvector result has no detectable advantage over FAISS under matched queries. Evidence: Figure~\ref{fig:s3-paired-audit} and Table~\ref{tab:s3-paired-quality} report a matched-query audit over the 5{,}000-query S3 multi-evidence matched-audit subset in which pgvector trails the FAISS exact FlatIP baseline by 0.0001 \ecov{} with paired 95\% interval [-0.0003, 0.0000].
  ...
  \item Claim: corpus-size stress testing changes the interpretation from a universal FAISS-exact ranking to a local-regime result. Evidence: Table~\ref{tab:faiss-scale-stress} reports a controlled FAISS vector-scale check where exact FlatIP remains below 200 ms p99 at 1M vectors, but IVF reaches p99 9.45 ms with recall@10 0.9994 against FlatIP; Table~\ref{tab:cross-engine-scale-stress} adds a fully indexed Qdrant HNSW sanity check on the same vector construction through 1M vectors.
